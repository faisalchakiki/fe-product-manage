...
function Navbar() {
  ...
  const addProductAction = (e) => {
    ...
    try {
      ...
      window.modalInsert.close();
      if (products.isLoading === false) {
        Swal.fire("Add Product Success!", "", "success").then((res) => {
          if (res.isConfirmed) {
            setTimeout(() => {
              navigate("/");
    ...
    <div className="bg-orange-600">
          ...
          <div className="flex-none gap-2">
            <div className="dropdown dropdown-end">
              ...
              >
                ...
                <li>
                  <p onClick={() => logoutAction()} className="py-3">
                    Logout
                  </p>
                </li>
              </ul>
      ...
      >
        <div className="modal-box md:rounded-b-[1rem] w-full md:w-10/12 lg:w-6/12 h-[50vh] relative md:h-auto scrollbar">
          <button onClick={() => window.modalInsert.close()} className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2 z-10">
            ✕
          </button>
